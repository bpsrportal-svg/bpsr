...
export async function GET() {
  ...
  const [contentsResult, modesResult, classesResult, rolesResult] = await Promise.all([
    supabase
      ...
      .select("id, category, name, description, sort_order, icon_url, is_active")
      .eq("is_active", true)
      .order("sort_order"),
    supabase
      .from("content_mode_masters")
      .select("id, content_id, name, sort_order, is_active")
      .eq("is_active", true)
      .order("sort_order"),
    supabase
      .from("class_masters")
      .select("id, name, role_key, sort_order, icon_url, is_active")
      .eq("is_active", true)
      .order("sort_order"),
